...
Luma lives **only** in this repo at `luma-user/`. Do not treat `luma-coincall-user` as a second project.
...
### Archive the old Luma GitHub repo
If Render still points at `rakizaofficial-png/luma-coincall-user`:
1. Reconnect that service to **CoinCall** with **Root Directory** = `luma-user`
2. On GitHub → that old repo → **Settings → Danger Zone → Archive repository**
...
- Open only this CoinCall folder in Cursor (see [README.md](README.md)).
